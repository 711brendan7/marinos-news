...
async def goto_row_page(page, reins_no, max_pages=30):
    """結果一覧で reins_no の行があるページまで送る（無ければ次ページへ）。
    まず1ページ目へ戻してから前方向に探す。
    重要: 検索結果は種目タブ分のページャ・行が同一DOMに同居するため、ページャ・行は
    すべて :visible で「現在表示中のタブ」に限定する（.first だと別タブの隠れ要素を掴み、
    戸建など特定種目だけ最終ページに居座って「行なし」を量産する）。
    ページャ操作は safe_click を使わない（不可視要素で scroll_into_view が30秒ハングするため
    素の click(timeout付き)＋try/except で回す）。
    """
